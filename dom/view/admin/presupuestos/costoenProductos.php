...
<div class="col-md-3">
<select  class="form-control" type="textarea" id="descripcion" name="idProducto" placeholder="Descripcion general del presupuesto" required>
...
<?php while($rows= mysql_fetch_array($sql)){ ?>
  ...
  <label for="example-search-input" class="col-xs-1 col-form-label">Presio $</label>
   <div class="col-md-2">
    <input class="form-control" type="text" id="presio" placeholder="0.00" name="presio">
  </div>
...
<div class="row">
  ...
  <tbody>
  <?php 
  $sql2 = mysql_query("SELECT materiales.cantidad, productos.descripcion, productos.precio, materiales.montoTotal FROM Materiales, productos WHERE materiales.idPresupuesto='$idPresupuesto' and productos.idProducto=materiales.idProducto;" , $link) or die(mysql_error());
  ...
  <td><?php echo $rows2['montoTotal']; ?>.00</td>
  </tr>
  <?php } ?>
</tbody>
</table>
...
<script>
 function calcularMT(){
      ...
      if (isNaN(presio)){
       ...
       var montoTotal = cantidad*presio;
