...
$sql = mysql_query("SELECT * FROM proveedores, productos where idProducto='$idProducto';" , $link) or die(mysql_error());
$rows = mysql_fetch_array($sql);
$idProveedor=$rows['idProveedor'];
$sqlnombreProveedor= mysql_query("SELECT * FROM proveedores where idProveedor='$idProveedor';" , $link) or die(mysql_error());
...
$rows2 = mysql_fetch_array($sqlproveedores);
  ?>
<!DOCTYPE html>
<html>
<head>
...
<div class="row">
  ...
  <form class="registro"  action="../../../../controller/productos/actualizarProductos.php?idProducto=<?php echo $idProducto; ?>" method="POST" enctype="multipart/form-data" name="frmaltaProductos" id="frmRegistro">
...
<div class="form-group row">
   ...
   <option selected="selected" value="<?php echo $rows['marca']; ?>"><?php echo $rows['marca']; ?></option>
  ...
  <label for="example-search-input" class="col-xs-2 col-md-1 col-form-label">Precio</label>
   ...
   <div class="input-group"> <span class="input-group-addon"><i class="glyphicon glyphicon-usd"></i></span>
    <input class="form-control" type="text" id="precio" placeholder="0.00" name="precio" onKeyPress="return solonumeros(event)" value="<?php echo $rows['precio'];?>">
  </div> 
</div>
</div>
</div>
<div class="form-group row">
  ...
  <div class=" col-xs-10 col-md-11">
   <div class="input-group"> <span class="input-group-addon"><i class="glyphicon glyphicon-user"></i></span>
    <select  class="form-control" type="textarea" id="idProveedor" name="idProveedor">
      <option selected="selected" value="<?php echo $idProveedor; ?>"><?php echo $nombreProveedor; ?></option>
      <?php                   
      while ($rows3 = mysql_fetch_array($sqlproveedores)){ ?>
      <option value="<?php echo $rows2['idProveedor'];?>"><?php echo $rows2['nombreProveedor']; ?></option>
      <?php } ?>
    </select>
  </div> 
</div>
...
$('#frmRegistro').bootstrapValidator({
   /*feedbackIcons: {
    ...
    fields: {
        ...
        idProveedor: { validators: { notEmpty: { message: 'Selecciona un proveedor' } } },
    }
